...
from pymodaq_plugins_keithley.hardware.keithley2700.keithley2700_VISADriver import Keithley2700VISADriver as Keithley
...
class DAQ_0DViewer_Keithley2700(DAQ_Viewer_base):
    ...
    DAQ_Viewer_base. It makes a bridge between the DAQ_Viewer module and the keithley2700_VISADriver.

    :param controller: The particular object that allow the communication with the keithley2700_VISADriver.
    ...
    for instr in config["Keithley", "2700"].keys():
        if "INSTRUMENT" in instr:
            resources_list += [config["Keithley", "2700", instr, "rsrc_name"]]
        ...
        if self.settings.child('controller_status').value() == "Slave":
            ...
            try:
                ...
                for instr in config["Keithley", "2700"]:
                    if "INSTRUMENT" in instr:
                        if config["Keithley", "2700", instr, "rsrc_name"] == self.settings["resources"]:
                            self.rsrc_name = config["Keithley", "2700", instr, "rsrc_name"]
                            self.panel = config["Keithley", "2700", instr, "panel"].upper()
                            self.instr = instr
