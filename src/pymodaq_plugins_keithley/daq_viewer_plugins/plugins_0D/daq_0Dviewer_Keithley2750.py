...
from pymodaq_plugins_keithley.hardware.keithley2750.keithley2750_VISADriver import Keithley2750VISADriver as Keithley
from pymodaq.utils.logger import set_logger, get_module_name
logger = set_logger(get_module_name(__file__))


class DAQ_0DViewer_Keithley2750(DAQ_Viewer_base):
    """ Keithley plugin class for a OD viewer.

    This object inherits all functionalities to communicate with PyMoDAQ’s DAQ_Viewer module through inheritance via
    DAQ_Viewer_base. It makes a bridge between the DAQ_Viewer module and the keithley2750_VISADriver.

    :param controller: The particular object that allow the communication with the keithley2750_VISADriver.
    ...
    for instr in config["Keithley", "2750"].keys():
        if "INSTRUMENT" in instr:
            resources_list += [config["Keithley", "2750", instr, "rsrc_name"]]
        ...
        if self.settings.child('controller_status').value() == "Slave":
            ...
            try:
                ...
                for instr in config["Keithley", "2750"]:
                    if "INSTRUMENT" in instr:
                        if config["Keithley", "2750", instr, "rsrc_name"] == self.settings["resources"]:
                            self.rsrc_name = config["Keithley", "2750", instr, "rsrc_name"]
                            self.panel = config["Keithley", "2750", instr, "panel"].upper()
                            self.instr = instr
